...
import membershipplan from '../components/menbership-plan/membership-plan';
...
const Account = ({ subscription, }: AccountProps) => {
	...
	return (
		<>
			...
			<header className=' bg-black/80 md:bg-[#141414]'>
			...
			<main className='mx-auto space-y-3  max-w-6xl px-5 pt-24 pb-12 mt-5 md:mt-0 transition-all md:px-10'>
				...
				<div className='mt-6 space-y-2  grid grid-cols-1 gap-x-4 border px-4 py-4 md:grid-cols-4 md:bordder-x-0 md:border-t md:border-b-0 md:pb-0'>
				...
				<div className='mt-6 grid space-y-2 grid-cols-1 gap-x-4 border px-4 py-4 md:grid-cols-4 md:bordder-x-0 md:border-t md:border-b-0 md:pb-0'>
					<h4 className='text-lg text-[gray]'>Settings</h4>
					<p onClick={logOut} className='col-span-3 cursor-pointer text-blue-500 hover:underline'>Sign out of all devices</p>
				</div>
			</main>
		</>
	);
...
export const getServerSideProps: GetServerSideProps<AccountProps> = async ({ req }) => {
	...
	const subscription =await fetch(`${API_REQUEST.subscraption}/${user_id}`).then(res => res.json());
